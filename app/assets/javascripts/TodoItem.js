...
TodoItem.prototype = {
  toHtml: function() {
  todo = $('<tr>').data('id', this.id).append($('<td>').text(this.name));
  if (this.createdAt === this.completedAt) {
    todo.addClass('unfinished-item').append($('<td>').addClass('todo-time').text(this.createdAt));
  }
  else {
    todo.addClass('finished-item').append($('<td>').addClass('todo-time').text(this.completedAt));
  }
